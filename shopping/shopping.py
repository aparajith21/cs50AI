...
def load_data(filename):
    ...
    with open(filename) as f:
        file_lines = list(csv.DictReader(f))
        for line in file_lines:
            line['Month'] = months[line['Month']]
            line['Weekend'] = 1 if line['Weekend'] == 'TRUE' else 0
            line['VisitorType'] = 1 if line['VisitorType'] == 'Returning_Visitor' else 0
            for column in [list(line.keys())[index] for index in [0, 2, 4, 11, 12, 13, 14]]:
                line[column] = int(line[column])
                
            for column in [list(line.keys())[index] for index in [1, 3, 5, 6, 7, 8, 9]]:
